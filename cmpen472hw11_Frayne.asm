...
newCmd      
            ldaa    #'>'
            jsr     putchar            ; print characters that start every line for a command
            ldaa    #' '
            jsr     putchar
            ldx     #BUF
                                       ; load address of input buffer
            ldab    #0                 ; initialize counter that tracks how many characters have been typed 

mloop
            jsr     updateTime         ; wait for input and check if time needs to be updated between each check
            jsr     getchar
            cmpa    #0                 ; loop if no serial input
            beq     mloop
            
            
            cmpb    #11
            bge     invalidInput      ; invalid input for 11 characters or more
            addb    #1
            staa    1,X+
            ...
            ldaa    #$00            ; replace enter character with null terminator for printing of buffer
            staa    X
            
            
            jsr     parseCmd        ; parse command
            
            bra     newCmd          ; loop to wait for a new command

          
invalidInput
            jsr     nextline           ; print invalid input message
            ldx     #invalidInputErr
            jsr     printmsg
            jsr     nextline
            bra     newCmd
...
;***************generateWave**********************
;* Program: prints numbers to terminal that create a wave when plotted
;* Input:    datamax, ctr125u
;* Output:  2048 decimal digits printed to terminal
;* Registers modified:x,a 
;* Algorithm:
;            print reminder to user to save output
;             begins wave generation after enter key
;           starts timer
;            continually check if time needs to be updated until ctr125u has reached datamax - timer interupt runs correct subroutine to generate wave
;            print done message when 2048 data points have been printed to terminal
;**********************************************
generateWave
             pshd
             pshx
             ...
             ldx     #msg3
             jsr     printmsg
             jsr     nextline            ; prints reminder for user to save termianl output
...
;***************gwSub**********************
;* Program: generate sawtooth wave
;* Input: ctr125u
;* Output: decimal number printed to terminal, ctr125u
;* Registers modified:d,x 
;* Algorithm: increase ctr125u and print lower 8 bits
;**********************************************
gwSub
...
;***************gw2Sub**********************
;* Program: generate sawtooth wave of 100Hz
;* Input:  ctr125u, sw2Ctr
;* Output: decimal number printed to terminal,gw2Ctr,ctr125u updated
;* Registers modified: d,x 
;* Algorithm:
;            print 80 increasing data points then reset
;**********************************************
gw2Sub
            ldd   #3000              ; 125usec with (24MHz/1 clock)
            addd  TC4H               ;    for next interrupt
            ...
            ldab  gw2Ctr
            incb
            stab  gw2Ctr                 ;uses counter to print 80 increaseing data points and reset to 0 after 80 points
            cmpb  #80
            bne   gw2print
            ldab  #0
            stab  gw2Ctr
...
gw2print                                ;   print ctr125u, only the last byte 
            clra
            jsr   pnum10             ;   to make the file RxData3.txt with exactly 1024 data 
gw2done     RTI
...
;***************gtSub**********************
;* Program: generate triangle wave data 
;* Input: triWaveDirection,triCtr,ctr125u
;* Output: decimal number printed to terminal, triWaveDirection,triCtr,ctr125u updated
;* Registers modified: d,x
;* Algorithm:
;              increase for 255 points then decrease and repeat
;**********************************************
gtSub
             ldd   #3000              ; 125usec with (24MHz/1 clock)
             addd  TC4H               ;    for next interrupt
             ...
             ldab   triWaveDirection     ; load wave direction value
             bne    triSubtract          ; if 0 then increase wave, if anything else decrease 
             
             ldd    triCtr        
             addd   #1                   ; increase wave data
             std    triCtr
             bra    flip   
             
triSubtract  
             ldd    triCtr
             subd   #1                  ; decrease wave data
             std    triCtr                   
...
flip
             ldd    triCtr
             subd   #1
             cpd    #254                 ;check if enough data points have been printed to switch direction that wave is changing
             blo    triend
             
             ldab   triWaveDirection
             comb                        ; flips to opposite wave directions
...
triend             
             ldx   ctr125u
             inx                         ; increase timer counter
             stx   ctr125u
             
             ldd   triCtr               ; print wave data
             jsr   pnum10
             rti
...
;***************gqSub**********************
;* Program: generate square wave with 510 points per wave period
;* Input: 
;* Output: decimal number printed to terminal, sqWaveCtr and ctr125u updated
;* Registers modified:d,x 
;* Algorithm:
;               load timer ctr
;               check lower 8 bits of ctr reached $FF
;                  if so flip output
;               print output to terminal as decimal number
;    
;**********************************************
gqSub
             ldd   #3000              ; 125usec with (24MHz/1 clock)
             ...
             ldab  sqWaveOut
             comb                    ; flips output from high to low or low to high every 255 data points
             stab  sqWaveOut
             
donegq       
             inx
             stx    ctr125u          ; increase counter and print data value
             ldab   sqWaveOut
             clra 
             jsr   pnum10
             rti
...
;***************gq2Sub**********************
;* Program: generates square wave of 100Hz frequency
;* Input: sqWaveCtr, sqWaveOut
;* Output: decimal number printed to terminal, sqWaveCtr and ctr125u updated
;* Registers modified: d,x 
;* Algorithm:
;             increment sqWaveCtr
;             check if sqWaveCtr has reached 40
;             if so:
;                   reset sqWaveCtr
;                   invert wave data
;             increment timer ctr
;             print wave data in decimal ascii
;**********************************************
gq2Sub
             ldd   #3000              ; 125usec with (24MHz/1 clock)
             addd  TC4H               ;    for next interrupt
             ...
             ldaa   sqWaveCtr
             inca
             staa   sqWaveCtr      ; update counter until it reaches 40 to print 40 points until switching to create 100 Hz wave
             cmpa   #40
             bne    donegq2
             ldaa   #0
             staa   sqWaveCtr
             ...
             ldab   sqWaveOut
             comb                   ; flip all bits to switch between 0 and 255 for output
             stab   sqWaveOut
             
donegq2       
             ldx    ctr125u
             inx                    ; increase timer counter to print 2048 data points
             stx    ctr125u
             ldab   sqWaveOut
             clra 
             jsr    pnum10         ; print number in decimal format
             rti
...
minutesNumber
            ...
            ; seconds number
            ldaa    1,X+                   ; do same thing as for seconds number
            suba    #$30
            cmpa    #9
            lbhi    invalidTime
            staa    inputSecond
...
checkg                                     ; check for gw command
            ...
            ldx     #gwSub                 ; load subroutine to generate sawtooth wave to timer interupt address
            stx     $FFE6
            jsr     generateWave
            lbra     endParse    
...
checkgw2    
            cmpa    #'2'                  ; check for gw2 command
            lbne     invalidCmd
            ldx     #gw2Sub
            stx     $FFE6
            ldaa    #-1
            staa    gw2Ctr                 ;initialize counter for sawtooth wave to get specific frequency
            jsr     generateWave           ; load subroutine to generate 100 Hz sawtooth wave to timer interupt address
            bra     endParse
            
checkgt                                  ; check for gt command
            cmpa    #'t'
            bne     checkgq
            ldaa    1,X+
            cmpa    #CR
            bne     invalidCmd
            ldx     #gtSub
            stx     $FFE6                   ; load subroutine to generate triangle wave to timer interupt address
            jsr     generateWave
            bra     endParse
            
checkgq                                    ; check for gq command
            cmpa    #'q'
            bne     invalidCmd
            ...
            bne     checkgq2
            ldx     #gqSub
            stx     $FFE6                  ; load subroutine to generate square wave to timer interupt address
            jsr     generateWave
            bra     endParse    
            
            
checkgq2                                   ; check for gq2 command
            cmpa    #'2'
            bne     invalidCmd
            ldx     #gq2Sub
            stx     $FFE6
            ldaa    #0                       ;initialize counter for square wave to get specific frequency
            staa    sqWaveCtr                ; load subroutine to generate 100 Hz square wave to timer interupt address
            jsr     generateWave
            bra     endParse    
            
            
checkA      
            cmpa    #'a'                    ; check if input was adc
            bne     checkQ                  ; move to check for q if the first letter isn't a
            ldaa    1,X+
            cmpa    #'d'
            bne     invalidCmd              ; if first letter is a but second and third letter aren't d and c then invalid cmd
            ldaa    1,X+
            cmpa    #'c'
            bne     invalidCmd 
            ldaa    1,X+
            ...
            ldx     #go2ADC                  ; load address for subroutine to convert analog signal to digital
            stx     $FFE6                    ; store subrtouine address into timer interupt address
            jsr     generateWave             ; generate wave
            bra     endParse
...
endParse            
            ...
            ldaa    #'>'                      ;print indeicator for user input
            jsr     putchar
            ldaa    #' '
            jsr     putchar
            jsr     nextline
...
invalidCmd  
            ...
            ldx     #invalidCmdErr             ;print invalid input message
            jsr     printmsg
            jsr     nextline
...
invalidTime
            ldx     #timeFormatErr
            jsr     printmsg                    ; print invalid time format message
            jsr     nextline
            puld
            pulx
            rts
...
;**********************************************
typeWriter      
                jsr   nextline            ;prints instructions for type writer
                ldx   #typeWriterMsg
                jsr   printmsg
                jsr   nextline
...
go2ADC
            ...
            LDAA  #%10000100       ; right justified, unsigned, single conversion,
...
msg1        DC.B   'Hello, you may used the following commands',CR,'t - sets the time',CR,'s - display seconds on clock',CR,'m - display minutes on clock',CR,'h - display hours on clock',CR,'q - quit program and strat typeWriter program',CR,'gw - generate sawtooth wave',CR,'gw2 - generate sawtooth wave at 125Hz',CR,'t - sets the time',CR,'gt - geneerate triangle wave',CR,'gq - generate square wave',CR,'gq2 - generate square wave at 125Hz',CR,'adc - convert analog signal to digital signal.(you must connect an analog signal to port to ADC port 4 before running)',CR,'Ensure you save the terminal output to file before starting any wave generations', $00                                                     
